...
import com.example.data.calladapter.enqueue
...
import com.example.data.calladapter.error.Error as ApiError
...
class ApiResultCall<T>(
    ...
    private fun Response<T>.toApiResult(): ApiResult<T> {
        if(!isSuccessful) {
            ...
            return Err(ApiError.HttpError(httpCode, errorResponse))
        }
